...
本文件是 monitor-trainning skill 同脚本（.claude/skills/monitor-trainning/src/
prune_checkpoints.py，仅存在于本机、不入库）的服务器可运行副本：train.py 自身不做
清理，训练期间需要它在服务器上按小时轮询裁剪 model_state，否则 optimizer.pt
（~6.6G/个）会填满 /cloud/cloud-ssd1。改动请两边同步。
...
用法：
  python tools/prune_checkpoints.py --output_dir <OUT> [--keep_model_state 3] \
      [--keep_weights N] [--keep_always 18000,30000] [--dry-run]
  # --keep_always: 永不清理的 step 逗号列表（各阶段最后一个节点 model_state，如跨阶段交接点）。
轮询（每 1 小时，服务器上 nohup 起）：
  nohup bash -c 'while :; do <python> tools/prune_checkpoints.py --output_dir <OUT> \
      --keep_model_state 3 >> <OUT_DIR>/../prune_loop.log 2>&1; sleep 3600; done' &
"""
...
def prune(output_dir: Path, keep_model_state: int, keep_weights: int | None,
          min_age: float, dry_run: bool, keep_always: set[int] | None = None) -> int:
    """执行清理，返回删除的目录数。

    keep_always: 永不清理的 step 集合（如每个阶段最后一个节点的 model_state，
    用于跨阶段 resume/回退的交接点）。
    """
    removed = 0
    protected = keep_always or set()
    is_protected = lambda d: _step(d) in protected
    ...
    if keep_weights is not None:
        for d in weights[keep_weights:]:
            if is_protected(d):
                print(f"  [skip] {d} (protected, keep_always)")
                continue
...
def main() -> int:
    ...
    ap.add_argument("--keep_always", type=str, default="",
                    help="永不清理的 step 逗号列表（各阶段最后一个节点的 model_state，如 18000,30000）")
    args = ap.parse_args()
    keep_always = {int(s) for s in args.keep_always.split(",") if s.strip()}
    prune(Path(args.output_dir), args.keep_model_state, args.keep_weights,
          args.min_age, args.dry_run, keep_always)
