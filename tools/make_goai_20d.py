...
转换（与 organizer 生成的 lerobot_v30_ee_6d 一致，已在 5 个 episode 上交叉验证）：
    每臂 [xyz(3), quat_wxyz(4), g(1)] -> [xyz(3), rot6d(6), g(1)]
    - quat_wxyz -> rotate6d（scalar_first=True）
    - gripper 默认反转：1-g（对齐 X-VLA-Pt EE6D 约定 1=闭合）；
      加 --no-invert-gripper 可保留原始 g 不变
    - 20 维布局：[l_xyz, l_rot6d, l_g, r_xyz, r_rot6d, r_g]

用法：
    python tools/make_goai_20d.py <src_root> <dst_root> [--no-invert-gripper]
...
def convert_16_to_20(arr: np.ndarray, invert_gripper: bool = True) -> np.ndarray:
    """[..., 16] -> [..., 20]；每臂 xyz+quat_wxyz+g -> xyz+rot6d+g。

    invert_gripper=True 时 g -> 1-g（对齐 X-VLA-Pt EE6D 约定 1=闭合）；
    False 时保留原始 g 值。
    """
    ...
    if invert_gripper:
        ...
        r_g = right[..., 7:8]
    l = np.concatenate(
        [left[..., :3], quat_to_rotate6d(left[..., 3:7], scalar_first=True), l_g], -1
    )
    r = np.concatenate(
        [right[..., :3], quat_to_rotate6d(right[..., 3:7], scalar_first=True), r_g], -1
    )
    return np.concatenate([l, r], -1).astype(np.float32)


def rewrite_parquet(src: Path, dst: Path, invert_gripper: bool = True) -> tuple[np.ndarray | None, np.ndarray | None]:
    ...
    for field in table.schema:
        ...
        if name in ("observation.state", "action"):
            ...
            rows = convert_16_to_20(rows, invert_gripper=invert_gripper)
...
def main() -> None:
    ...
    for src in sorted(src_root.glob("data/**/file-*.parquet")):
        ...
        s, a = rewrite_parquet(src, dst, invert_gripper=invert_gripper)
        if s is not None:
    ...
    print(f"done. 20-dim dataset written to {dst_root} (invert_gripper={invert_gripper})")
