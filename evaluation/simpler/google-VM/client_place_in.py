...
CONFIG_PATH = Path(__file__).parent / "configs/place_in.json"
    ...
    def step(self, image: np.ndarray) -> np.ndarray:
        ...
        if not self.action_plan:
            ...
            try:
                ...
                action_seq = np.array(result["action"], dtype=np.float32)[::2][:6] # action speedup and chunk cutting
        ...
        action_final = np.concatenate([
            ...
            np.array([1.0 if action_pred[9] > 0.28 else -1.0])
        ])
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="logs/place_in/",
